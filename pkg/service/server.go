...
func NewServer(
	...
	jwtKey string,
	jwtAlg []string,
	linkTokenExp time.Duration) (*Server, error) {
...
func (s *Server) repoHandler(w http.ResponseWriter, req *http.Request) {
	vars := mux.Vars(req)
...
func (s *Server) ListenAndServe(cert, key string) (err error) {
	...
	router.Handle("/", handlers.CompressHandler(func() http.Handler { return http.HandlerFunc(s.repoHandler) }())).
		Methods("GET")
	...
	if s.addrExt == nil {
		...
		if err != nil {
			return emperror.Wrapf(err, "cannot parse addr %s", addr)
		}
	}
